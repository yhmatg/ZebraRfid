...
public class LedFragment extends BackPressedFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Application.ledState = mSharedPreferences.getBoolean("LED_STATE1", true);

        context = rootview.getContext();
        //Application.AUTO_DETECT_READERS = Ledstate.ge(Constants.AUTO_DETECT_READERS, true);
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (checkboxled.isChecked()) {
                    if (Application.mConnectedReader != null && Application.mConnectedReader.isConnected())
                        try {
                            Application.mConnectedReader.Config.setLedBlinkEnable(true);
                    ...
                    try {
                        if (Application.mConnectedReader != null) {
                            Application.mConnectedReader.Config.setLedBlinkEnable(false);
                        }
                    } catch (InvalidUsageException e) {
                        e.printStackTrace();
                    } catch (OperationFailureException e) {

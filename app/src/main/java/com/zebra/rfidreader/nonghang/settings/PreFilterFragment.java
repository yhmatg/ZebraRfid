...
import com.zebra.rfidreader.nonghang.common.asciitohex;
import com.zebra.rfidreader.nonghang.common.hextoascii;
import com.zebra.rfidreader.nonghang.home.MainActivity;

import static com.zebra.rfidreader.nonghang.application.Application.isLocatingTag;
import static com.zebra.rfidreader.nonghang.application.Application.mIsInventoryRunning;
import static com.zebra.rfidreader.nonghang.home.MainActivity.filter;
import static com.zebra.rfidreader.nonghang.settings.PreFilter1ContentFragment.setSingulation;
...
public class PreFilterFragment extends BackPressedFragment {
    ...
    private boolean CheckForSimpleFilterSwitch() {
        if (Application.preFilters != null && Application.preFilters[1] != null)
            return false;
        if (Application.preFilters[0] != null && (!Application.preFilters[0].getMemoryBank().equals("EPC") || Application.preFilters[0].getAction() != 4))
            return false;
        return true;
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (preFilters1 != null) {
            ...
            Application.preFilters[0] = new PreFilters(preFilters1.getStringTagPattern(), memoryBank, preFilters1.getBitOffset() / 16, preFilters1.getTagPatternBitCount(), preFilters1.StateAwareAction.getStateAwareAction().getValue(),
                    target, true);
        ...
        if (preFilters2 != null) {
            ...
            if (preFilters2.StateAwareAction.getTarget().getValue() == 1)
                ...
                target = 4;
            Application.preFilters[1] = new PreFilters(preFilters2.getStringTagPattern(), memoryBank, preFilters2.getBitOffset() / 16, preFilters2.getTagPatternBitCount(), preFilters2.StateAwareAction.getStateAwareAction().getValue(),
                    target, true);
        }
    ...
    private void initializeSimplePrefilterView() {
        ...
        if (Application.preFilters != null && Application.preFilters[0] != null) {
            if (Application.preFilters[0].getMemoryBank().equals("EPC") && Application.preFilters[0].getAction() == 4 && Application.preFilters[1] == null) {
                ...
                if (Application.PreFilterTagID != null && !Application.PreFilterTagID.equals("") && Application.PreFilterTagID.toUpperCase().startsWith(tag.substring(0, tag.length() - 1), start)) {
                    tag = Application.PreFilterTagID;
                    if (start <= tag.length())
                        bSelectID = true;
                }
        ...
        if (bSelectID) {
            String tagid = Application.preFilters[0].getTag();
            stop = start + Application.preFilters[0].getBitCount() / 4;//tagid.length();
            if ((Application.PreFilterTagID + "0").endsWith(tagid))
                stop = Application.PreFilterTagID.length();
            if (Application.asciiMode == true && tagIDField.getText().toString().startsWith("'")) {
    ...
    public boolean issettingsChanged() {
        ...
        if (getActivity().findViewById(R.id.simplefilter).getVisibility() == View.VISIBLE) {
            ...
            if (Application.preFilters == null)
                return false;
            if ((Application.preFilters[0] == null && ((CheckBox) getActivity().findViewById(R.id.preFilterEnableFilterSimple)).isChecked())) {
                prefilterCombination = 1;
                return true;
            } else if ((Application.preFilters[0] != null && !((CheckBox) getActivity().findViewById(R.id.preFilterEnableFilterSimple)).isChecked())) {
                prefilterCombination = 1;
                deletePrefilter1 = true;
                return true;
            } else if ((Application.preFilters[0] != null) && ((CheckBox) getActivity().findViewById(R.id.preFilterEnableFilterSimple)).isChecked()) {
                String tagID = Application.asciiMode == true ? AsciitohexConvert(tagIDField.getText().toString(), offset, length) : tagIDField.getText().toString().substring(offset, length);
//                if (!tagIDField.hasSelection()) {
//                    offset = (Application.preFilters[0].getOffset() * 16 - 32) / 4;
//                }
                if (Application.preFilters[0].getTag().length() * 4 != Application.preFilters[0].getBitCount()) {
                    tagID = (tagID + "0");
                }
                if (Application.asciiMode && tagIDField.getText().toString().startsWith("'")) {
                    if (offset != 0)
                        offset -= 1;
                    offset *= 2;
                }
                if (Application.PreFilterTagID == null || Application.PreFilterTagID.equals(""))
                    offset = (Application.preFilters[0].getOffset() - 2) * 4;
                PreFilters preFilterCurrent = new PreFilters(tagID,
                        "EPC",
                        (offset * 4 + 32) / 16,
                        0, 4,
                        Application.singulationControl.getSession().getValue(), true);
    ...
    void fillPrefilter(com.zebra.rfid.api3.PreFilters.PreFilter preFilterPassed1, com.zebra.rfid.api3.PreFilters.PreFilter preFilterPassed2) {
        if (Application.mConnectedReader != null && Application.mConnectedReader.isConnected()) {
            ...
            if (getActivity().findViewById(R.id.simplefilter).getVisibility() == View.VISIBLE) {
                ...
                if (Application.asciiMode == true)
                    putTagPattern(preFilterPassed1, Application.preFilters[0], AsciitohexConvert(tagIDField.getText().toString(), offset, length));
                else
                    putTagPattern(preFilterPassed1, Application.preFilters[0], (tagIDField.getText().toString().substring(offset, length)));
                if (Application.asciiMode == true && tagIDField.getText().toString().startsWith("'") && tagIDField.getText().toString().endsWith("'")) {
                    if (tagIDField.hasSelection()) {
                ...
                putOffSet(preFilterPassed1, Application.preFilters[0], offset * 4 + 32);
                preFilterPassed1.setTagPatternBitCount((length - offset) * 4);
                putMemoryBank(preFilterPassed1, Application.preFilters[0], MEMORY_BANK.MEMORY_BANK_EPC);
                preFilterPassed1.setFilterAction(FILTER_ACTION.FILTER_ACTION_STATE_AWARE);

                putTarget(preFilterPassed1, Application.preFilters[0], GetTargetForSession());
                putAction(preFilterPassed1, Application.preFilters[0], STATE_AWARE_ACTION.STATE_AWARE_ACTION_INV_B_NOT_INV_A);
                ...
                if ((enablePreFilter.isChecked() && offsetText.isEmpty()) || (enablePreFilter2.isChecked() && offsetText2.isEmpty())) {
                    ...
                    if (enablePreFilter.isChecked()) {
                        ...
                        putTagPattern(preFilterPassed1, Application.preFilters[0], data);
                        putOffSet(preFilterPassed1, Application.preFilters[0], offset * Constants.NO_OF_BITS);
                        ...
                        if (memBank != null) {
                            putMemoryBank(preFilterPassed1, Application.preFilters[0], memBank);
                        }
                        ...
                        if (target != null) {
                            putTarget(preFilterPassed1, Application.preFilters[0], target);
                        }
                        ...
                        if (action != null) {
                            putAction(preFilterPassed1, Application.preFilters[0], action);
                        }
                    }
                    ...
                    if (enablePreFilter2.isChecked()) {
                        ...
                        putTagPattern(preFilterPassed2, Application.preFilters[1], data);
                        putOffSet(preFilterPassed2, Application.preFilters[1], offset * Constants.NO_OF_BITS);
                        ...
                        if (memBank != null) {
                            putMemoryBank(preFilterPassed2, Application.preFilters[1], memBank);
                        }
                        ...
                        if (target != null) {
                            putTarget(preFilterPassed2, Application.preFilters[1], target);
                        }
                        ...
                        if (action != null) {
                            putAction(preFilterPassed2, Application.preFilters[1], action);
                        }
                    }
                }
            }
    ...
    private TARGET GetTargetForSession() {
        ...
        if (Application.singulationControl != null) {
            SESSION session = Application.singulationControl.getSession();
    ...
    private class Task_SavePrefilter extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                if ((prefilterCombination == 1) && !deletePrefilter1 && !deletePrefilter2) {
                    ...
                    Application.preFilters[0] = new PreFilters(PreFilterData1);
                    ...
                    Application.preFilters[1] = new PreFilters(PreFilterData2);
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (getActivity().findViewById(R.id.simplefilter).getVisibility() == View.VISIBLE) {
                final AutoCompleteTextView tagIDField = ((AutoCompleteTextView) getActivity().findViewById(R.id.preFilterTagIDSimple));
                Application.PreFilterTagID = Application.asciiMode == true ? asciitohex.convert(tagIDField.getText().toString()) : tagIDField.getText().toString();
            }
            if (!result) {
